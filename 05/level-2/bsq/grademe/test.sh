...
# ---------------------------------------------------------------------------- #
#  Funciones NO permitidas por "Allowed functions and globals": sscanf
#  (solo fscanf esta en la lista — parsear un string ya leido con getline
#  usando sscanf es una violacion facil de colar) e isprint (no esta en
#  la lista tampoco, pese a ser tentador para validar la cabecera).
# ---------------------------------------------------------------------------- #
check_forbidden_functions() {
    echo -e "${CYAN}${BOLD}=== Funciones NO permitidas (sscanf, isprint) ===${RESET}"
    ...
    if [ -z "$hits" ]; then
        pass "Ni sscanf ni isprint aparecen como codigo (solo fscanf/chequeo manual permitidos)"
    else
        fail "Uso de funcion NO permitida (fuera de comentarios):$hits"
    fi
    echo ""
}
